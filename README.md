# 입자 압축 (Particle Compression) — 데스크탑 위젯 방치형

> 바탕화면 구석에서 물질이 압축되고 있다. 분자에서 플랑크 길이까지 내려가며 미지의
> 입자를 발견하는 **데스크탑 방치형(위젯 아이들러) × 인크리멘탈**. **작아짐 = 강해짐.**

**2-모드 단일 앱** (GDD v0.4 컨셉 피벗):
- **위젯 모드(기본, 관조)** — 투명·프레임리스·always-on-top 소형 창이 바탕화면에 상주.
  현재 층 세계+게임판을 게임과 **동일 렌더로 동기화** — 러스티의 은퇴처럼 곁에 두고 지켜본다.
- **게임 모드(개입)** — 일반 창으로 변형해 전체 UI(PoE식 연구 트리·도감 87입자·상전이·설정).
  Esc/트레이/설정으로 전환.

웹(데모·플레이테스트) + PC Steam(Tauri v2 + steamworks-rs) 단일 코드베이스.
이 저장소는 게임 **코드**와 `design/` **설계 문서**를 함께 보관한다. 설계는 단일 진실
소스(`design/GDD.md` 외)이며, 코드는 그 설계를 충실히 구현한다.

현재 상태: **정거장2+ — 위젯 컨셉 피벗(v0.4) 구현 완료.** 위젯(세계 동기화·투명창·드래그·
트레이·창 변형 전환·비포커스 스로틀·호버 HUD·입력 격리) + 다이제틱 공허 게임판(11 세계·
코어·8 궤도 껍질·능동 메커니즘 4종 직접 만지기) + 빅 크런치/재하강 + PoE식 연구 트리 +
절차적 WebAudio 사운드 + 설정/기록/업적/도움말/도감 필터. 코어 루프·경제·밸런스는
구현·검증 완료(경제 수치 0 변경 원칙). Steam 연동(steamworks-rs) = M3.7 잔여.
방향 SSOT = [`design/cosmic-direction.md`](design/cosmic-direction.md) · 제품 정의 = `design/GDD.md` §2.
