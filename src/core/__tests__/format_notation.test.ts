...
describe('formatMultiplier / formatPercent (AD format.js formatX·formatPercents 참조)', () => {
  it('배율은 항상 고정 소수(정수라도 ×N.NN — toFixed/formatNumber 조립 통일)', () => {
    expect(formatMultiplier(1.005, 3)).toBe('×1.005');
    expect(formatMultiplier(1.35, 3)).toBe('×1.350');
    expect(formatMultiplier(0.82, 2)).toBe('×0.82');
    expect(formatMultiplier(2, 2)).toBe('×2.00');
  });
  it('거대 배율은 표기법으로 폴백', () => {
    expect(formatMultiplier('1e8', 2)).toBe('×1.00×10⁸');
  });
  it('퍼센트는 fraction×100, 기본 반올림 정수', () => {
