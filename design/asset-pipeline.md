...
> 에셋은 **절차생성 코드(최대)**, **오디오 스트리밍 파일(중간)**, **JSON 데이터(content-pipeline)** 세 범주로 나뉜다. 스프라이트·비트맵 에셋은 원칙적으로 없다. Tauri는 시스템 WebView2를 재사용하므로 별도 Chromium 번들이 없고 게임 번들 자체는 수~수십 MB(오디오 스템이 대부분)를 목표로 한다.
...
| 파비콘 / 앱 아이콘 (Steam 스토어, Tauri 윈도우 아이콘) | **파일 필요** | `PNG`/`.ico` 다중 크기. 절차생성 불가 |
...
**소문자·하이픈(kebab-case) 고정.** 공백·대문자·특수문자 금지(Tauri fs 경로 일관성).
...
> **⚠ v0.3 SUPERSEDED — Tauri 전환.** 아래 `package.nw`/`nwjs-sdk` 구조는 NW.js 기준(v0.2). Tauri는 Vite `dist/`를 실행파일에 번들(시스템 WebView2가 로드) → 별도 Chromium 합본·`package.nw` 없음. Tauri 번들 구조로 재기술 예정(tech-arch §5.1).
...
**오디오 파일 경로 주의:** Tauri(WebView) 환경에선 에셋이 Tauri asset/커스텀 프로토콜로 로드된다. Vite의 `new URL('./audio/ambient/l01_molecular/l01_base_drone.ogg', import.meta.url)` 패턴을 사용하여 빌드 타깃에 무관한 경로 해석을 보장한다.
...
| G8 | Tauri(WebView2)에서 에셋 프로토콜로 `fetch('audio/...')` 동작 검증 | 미검증 | graphics-programmer + game-programmer |
