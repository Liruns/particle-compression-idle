...
[
  ...
  },
  {
    ...
    "name_ko": "전체 티어 연쇄",
    "depth": 4,
    "effect_type": "모든 티어 연속 강화 활성화 — Tk 보유 수에 비례하여 Tk+1 생산 추가 보너스 (T1→T8 전체 적용, 계수 +0.01/개: T1 30개 보유 시 T2 생산에 +0.30× 추가)",
    "effect_value": "+0.01/개 (Tk 보유 수 × 계수 → Tk+1 생산 추가, production_mult 외부 체인 내부 배율)",
    "prerequisites": ["A9"],
    "gate_layer": 6,
    "gate_condition": "A9 구매 + A5 구매 + 프리온층(L6) 진입",
    "cost_D": "[ECONOMY]",
    "milestone_warning": false,
    "overlap_cap_note": "production_mult 상수곱 아님 — 체인 내부 연속 강화 배율(C안 채택). §3.4 상수곱 상한 계산 범위 외. [followup-systems.md §1-5 / director-review §3.4]",
...
[
  ...
  },
  {
    ...
    "name_ko": "3중 메커니즘 시너지",
    "depth": 3,
    "effect_type": "공명(§2-A) + 핵력 게이지(§2-B) + 삼원합일(§2-C) 세 메커니즘이 동시에 활성 상태일 때 dC/dt × 1.20 (조건: 공명 배율 활성 + 게이지 50% 이상 + 균형 임계값 달성, 지속 5초 초과 시 D +0.5 추가)",
    "effect_value": "dC/dt × 1.20 (조건부 이벤트 배율 — production_mult 상수곱 아님, 세 메커니즘 동시 활성 구간에만 적용)",
    "prerequisites": ["B4"],
    "gate_layer": 5,
    "gate_condition": "B4 구매 + 쿼크층(L5) 진입",
    "cost_D": "[ECONOMY]",
    "milestone_warning": false,
    "overlap_cap_note": "production_mult 상수곱 아님 — 조건부 dC/dt 이벤트 배율(C안 채택). 시간 평균 활성율 ~40% 가정 시 실효 ≈ 시간평균 ×1.08. §3.4 상수곱 상한 계산 범위 외. 힉스 보너스 노드 신설 금지(codex 힉스 보손 도감 보너스로 처리). [followup-systems.md §1-5 / director-review §3.4]",
  ...
  },
  {
    ...
    "name_ko": "완전 시너지",
    "depth": 3,
    "effect_type": "B 가지 시너지 이벤트 발동마다 D +[B12_D_bonus], QF +[B12_QF_bonus] 추가 트리클 — 조건: 도감 수집 80% 이상. 시너지 이벤트 경로를 통한 자원 증폭이며 production_mult 직접 상수곱 아님",
    "effect_value": "B 가지 시너지 이벤트 D·QF 트리클 ×1.5 (이벤트 경로 증폭 — production_mult 상수곱 아님)",
    "prerequisites": ["B10"],
    "gate_layer": 10,
    "gate_condition": "B10 구매 + 도감 수집 80% 이상",
    "cost_D": "[ECONOMY]",
    "milestone_warning": false,
    "overlap_cap_note": "production_mult 상수곱 아님 — B 가지 시너지 이벤트 D·QF 트리클 경로(C안 채택). §3.4 상수곱 상한 계산 범위 외. [followup-systems.md §1-5 / director-review §3.4]",
  ...
  },
  {
    ...
    "name_ko": "홀로그래픽 증폭",
    "depth": 2,
    "effect_type": "도감 수집 완성도에 따라 systems §2-I holographic_mult의 codex²항 계수를 강화 — 별도 holographic 가산항 추가 금지. 효과는 §2-I codex_bonus 곱 증폭으로 표현하며 holographic_mult ≤ ×1.35 상한 내에서 관리",
    "effect_value": "codex²항 계수 강화 (holographic_mult ≤ ×1.35 풀 안에서 도감 완성도 보너스와 합산 관리 — 별도 가산항으로 추가 시 ×1.454 → dec26 −31.2% FAIL)",
    "prerequisites": [],
    "gate_layer": 10,
    "gate_condition": "정보층(L10) 진입",
    "cost_D": "[ECONOMY]",
    "milestone_warning": false,
    "overlap_cap_note": "holographic_mult ×1.35 풀에 흡수됨 (C안 채택). 별도 holographic 가산항 신설 금지. [systems.md §2-I / followup-systems.md §1-5 / director-review §3.4]",
