...
platform 격리(Game이 캔버스 무지) 우선순위는 전적으로 옳다. (A)는 jsdom 테스트·NW.js에서 Game에 렌더러 분기를 강제하고 "Game=순수 로직" 경계를 깬다 → 기각 정당. rAF 1개 절약이 그 결합을 정당화 못 함.
...
- 본 슬라이스 부하(≤120 파티클·헤이즈 1·글로우 1)는 2D로 60fps 압도적 여유 — WebGL 추상화는 셰이더/컨텍스트로스트/이식(NW.js 드라이버) 복잡도만 추가(tech §6.3 "단순함이 안정성", §5.1 NW.js 단일 컨텍스트 정합).
