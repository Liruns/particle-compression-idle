...
### 6-K. `achievements` 네임스페이스 (M2.4d, 2026-07-08)

| 필드명 | 타입 | 의미 |
|---|---|---|
| `earned` | string[] | 달성한 관측 목표 ID 배열(Set→정렬 배열, codex/research와 동형 스파스 직렬화) |

> **구현:** 코드 `AchievementsState`(`core/state`) = `earned: Set<string>`. 정의는 데이터 주도(`data/achievements.ts`, 26개 — 첫걸음/하강/상전이/도감/연구/손맛/스케일 + 히든 2). 판정은 **영속 상태 파생**(`core/achievements` `evaluateAchievements`, 멱등). 세이브 봉투 **옵셔널 필드**(버전 업 없이 보존, 구버전=빈 집합).
> **★가드레일(GDD §13):** 마일스톤 배율(25/50/100→×2/×3) 금지 → 업적은 **순수 인정·목표·기록형(생산 배율 0)**. 검증된 경제 불변. Steam 업적(M3.7) 매핑 후보 — ID는 영구 계약(변경 금지, 추가만).
